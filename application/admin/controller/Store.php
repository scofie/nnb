...
class Store extends Base
{
    ...
    {
        // 查询产品
        $store = Db::table('store')->where('catalog', '<>', 1)->where('catalog', '<>', 2)->order('sort DESC')->paginate(10, false, ['query' => $req->param()]);
    ...
    {
        // 查询产品
        $store = Db::table('store')->where('catalog', '=', 1)->order('sort DESC')->paginate(10, false, ['query' => $req->param()]);
        $this->assign('store', $store);
        return $this->fetch();
    }
    ...
    {
        // 查询产品
        $store = Db::table('store')->where('catalog', '=', 2)->order('sort DESC')->paginate(10, false, ['query' => $req->param()]);
        $this->assign('store', $store);
        return $this->fetch();
    }
