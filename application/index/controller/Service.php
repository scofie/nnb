...
class Service extends Base
{
    ...
    public function sms(Request $req)
    {
        ...
        } else if (array_key_exists('other', $config)) {
            $result = $this->other_sms($mobile, $data[0], $data[1], $config['other']);
        } else if (array_key_exists('jisu', $config)) {
            $result = $this->jisu_sms($mobile, $data[0], $config['jisu']);
        }else {
     ...
     * 其他接口
     */
    private function other_sms($mobile, $captcha, $expire, $config)
    {
        // 签名
        $timestamp = date('YmdHis');
        $sig = md5($config['account_sid'] . $config['auth_token'] . $timestamp);
        // 参数
        $param = [
            ...
            'sig'           => $sig,
            'respDataType'  => 'JSON',
            'smsContent'    => '【AMA中国】您的验证码为' . $captcha . '，请于' . $expire . '内正确输入，如非本人操作，请忽略此短信。',
            'to'            =>  $mobile
        ];
        // 请求
        $url = $config['rest_url'] . '/industrySMS/sendSMS';
        $header = ['Content-type: application/x-www-form-urlencoded', 'Accept: application/json'];
        $param = http_build_query($param);
        // 响应
        $res = post($url, $param, $header);
        if ($res === false) {
            return [
                'code'          =>  501,
        ...
        if ($result['respCode'] == '00000') {
            ...
            return [
                'code'          =>  502,
                'message'       =>  $result['respDesc'] ?: $result['respCode']
            ];
        }
    }
     ...
     * 极速 -短信
     */
     private function jisu_sms($mobile,$captcha,$config){
     	$appkey = $config['appkey'];//你的appkey
		$mobile = $mobile;//手机号 超过1024请用POST
		$content = '您的手机验证码为'.$captcha.'，5分钟内有效。请不要把此验证码泄露给任何人。【ISS云盾】';//utf8
		...
		if($jsonarr['status'] != 0)
		{
			 return [
                'code'          =>  502,
                'message'       =>  $jsonarr['msg'] ?: $jsonarr['msg']
            ];
		} else{
			return true;
		}
