    ...
    public function scratch(Request $req)
    {
        ...
        if ($req->isPost()) {
            try {
                ...
                // 查询机会
                $myChance = config('hello.level.' . $user['account']['type'] . '.scratch') ?: 0;
                $chance = config('hello.event.scratch.chance');
                if (array_key_exists('people', $chance) && $chance['people'] > 0) {
                    // 直推已认证人数
                    $people = Db::table('account')->where('inviter', '=', $username)->where('authen', '=', 1)->count('id');
                    if ($people >= $chance['people']) {
                        $myChance += (int) floor($people / $chance['people']);
                    }
                    $myChance -= $count;
                    if ($myChance <= 0) {
                        return json([
                            'code'      =>  504,
                            'message'   =>  '很抱歉、您没有足够的机会！'
                        ]);
                    }
                    /*$cishu = (int) floor($people / $chance['people']);
                    if ($cishu <= 0) {
                        return json([
                            'code'      =>  504,
