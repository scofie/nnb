<?php /*a:2:{s:66:"/www/wwwroot/sl.g8ym.com/application/admin/view/wallet/record.html";i:1581830132;s:65:"/www/wwwroot/sl.g8ym.com/application/admin/view/common/world.html";i:1581830132;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
		...
		<div class="col-md-6 col-lg-4 mb-3">
			<div class="input-group">
				<select class="custom-select" name="currency">
					...
					<?php if(is_array(app('config')->get('hello.currencys')) || app('config')->get('hello.currencys') instanceof \think\Collection || app('config')->get('hello.currencys') instanceof \think\Paginator): $i = 0; $__LIST__ = app('config')->get('hello.currencys');if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$currency): $mod = ($i % 2 );++$i;if($key == app('request')->get('currency')): ?>
							<option selected="true" value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($currency['name']); ?></option>
						<?php else: ?>
		...
		<div class="col-md-6 col-lg-4 mb-3">
			<div class="input-group">
				<select class="custom-select" name="business">
					...
					<?php if(is_array(app('config')->get('hello.businesses')) || app('config')->get('hello.businesses') instanceof \think\Collection || app('config')->get('hello.businesses') instanceof \think\Paginator): $i = 0; $__LIST__ = app('config')->get('hello.businesses');if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$business): $mod = ($i % 2 );++$i;if(app('request')->get('business') == $key): ?>
							<option selected="true" value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($business); ?></option>
						<?php else: ?>
							<option value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($business); ?></option>
						<?php endif; endforeach; endif; else: echo "" ;endif; ?>
			    </select>
			</div>
		</div>
...
<div class="card">
	<div class="table-responsive">
	    <table class="table table-hover table-outline table-vcenter text-nowrap card-table">
	        ...
	        <tbody>
	        	<?php if(is_array($records) || $records instanceof \think\Collection || $records instanceof \think\Paginator): $i = 0; $__LIST__ = $records;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$record): $mod = ($i % 2 );++$i;?>
	            <tr>
	                <td class="text-center">
	                    <div class="avatar d-block" style="background-image: url(<?php echo avatar($record['avatar'], $record['idcard']); ?>)"></div>
	                </td>
	                <td>
	                    <div><?php echo htmlentities($record['nickname']); ?></div>
	                    <div class="small text-muted"><?php echo htmlentities($record['username']); ?></div>
	                </td>
	                <td><?php echo htmlentities($record['rid']); ?></td>
	                <td><?php echo htmlentities(app('config')->get('hello.currencys')[$record['currency']]['name']); ?></td>
	                <td><?php echo htmlentities(app('config')->get('hello.businesses')[$record['business']]); ?></td>
	                <td><?php echo htmlentities($record['before']); ?></td>
	                <td><?php echo htmlentities($record['now']); ?></td>
	                <td><?php echo htmlentities($record['after']); ?></td>
	                <td><?php echo htmlentities($record['create_at']); ?></td>
	            </tr>
	            <?php endforeach; endif; else: echo "" ;endif; ?>
	        </tbody>
	    </table>
	</div>
	<div class="card-footer"><?php echo $records; ?></div>
</div>
