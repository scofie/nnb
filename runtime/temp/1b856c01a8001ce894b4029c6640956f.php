<?php /*a:2:{s:66:"/www/wwwroot/sl.g8ym.com/application/admin/view/account/index.html";i:1581830132;s:65:"/www/wwwroot/sl.g8ym.com/application/admin/view/common/world.html";i:1581830132;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
	...
	<div class="row">
		<div class="col-md-6 col-lg-2 mb-3">
			<select class="form-control custom-select" name="type">
				...
				<?php if(is_array(app('config')->get('hello.level')) || app('config')->get('hello.level') instanceof \think\Collection || app('config')->get('hello.level') instanceof \think\Paginator): $i = 0; $__LIST__ = app('config')->get('hello.level');if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;if(is_null(app('request')->get('type'))): ?>
		...
		<div class="col-md-6 col-lg-3 mb-3">
			<div class="input-group">
				...
				<input type="text" class="form-control" name="inviter" value="<?php echo htmlentities(app('request')->get('inviter')); ?>" />
			</div>
		</div>
			...
			<div class="input-group">
				<div class="input-group-prepend">
					<select class="custom-select input-group-text" name="moneyField">
						<?php if(is_array($compares) || $compares instanceof \think\Collection || $compares instanceof \think\Paginator): $i = 0; $__LIST__ = $compares;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$text): $mod = ($i % 2 );++$i;if($key == app('request')->get('compare')): ?>
								<option value="<?php echo htmlentities($key); ?>" selected="true"><?php echo htmlentities($text); ?></option>
							<?php else: ?>
								<option value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($text); ?></option>
							<?php endif; endforeach; endif; else: echo "" ;endif; ?>
				    </select>
				</div>
				<input type="text" class="form-control" aria-label="Text input with dropdown button" name="moneyValue" value="<?php echo htmlentities(app('request')->get('moneyValue')); ?>" />
		...
		<div class="col-lg-4 mb-3">
			<div class="input-group">
				...
				<select class="custom-select input-group-text" name="sort">
					<?php if(is_array($sorts) || $sorts instanceof \think\Collection || $sorts instanceof \think\Paginator): $i = 0; $__LIST__ = $sorts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$text): $mod = ($i % 2 );++$i;if($key == app('request')->get('sort')): ?>
							<option value="<?php echo htmlentities($key); ?>" selected="true"><?php echo htmlentities($text); ?></option>
						<?php else: ?>
							<option value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($text); ?></option>
						<?php endif; endforeach; endif; else: echo "" ;endif; ?>
	        ...
	        <tbody>
	            ...
	            <tr>
	                ...
	                <td>
	                	...
	                	<div class="small text-muted">By: <?php echo htmlentities($user['inviter']); ?></div>
	                </td>
	                ...
	                <td>
	                	<?php switch($user['authen']): case "1": ?><span class="status-icon bg-success"></span>已实名<?php break; case "2": ?><span class="status-icon bg-warning"></span>待审核<?php break; case "3": ?><span class="status-icon bg-red"></span>被拒绝<?php break; default: ?>
	                ...
	                <td class="text-center">
	                	<?php if(app('request')->session('admin.adminname') == 'admin'): ?>
