<?php /*a:2:{s:69:"/www/wwwroot/sl.g8ym.com/application/admin/view/account/addadmin.html";i:1581830132;s:65:"/www/wwwroot/sl.g8ym.com/application/admin/view/common/world.html";i:1581830132;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
    ...
    <div class="page-single">
        <form class="card" method="post" style="max-width: 24rem;">
            <div class="card-header">
                <?php if($type == '1'): if(empty($id) || (($id instanceof \think\Collection || $id instanceof \think\Paginator ) && $id->isEmpty())): ?>
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="adminname" placeholder="用户名" required="true" value="<?php echo htmlentities((isset($admin['adminname']) && ($admin['adminname'] !== '')?$admin['adminname']:'')); ?>" <?php if($type == '2'): ?>readonly<?php endif; ?>/>
                </div>
                <div class="form-group">
                    <label class="form-label">密码</label>
                    <input type="text" class="form-control" name="psd" placeholder="密码" required="true" value="<?php echo htmlentities((isset($admin['password']) && ($admin['password'] !== '')?$admin['password']:'')); ?>" />
                </div>
                <div class="form-group" <?php if($type == '2'): ?>style="display:none;"<?php endif; ?>>
                    <label class="form-label">选择角色</label>
                    <select class="form-control custom-select" name="roleid" id="roleid">
                        <?php if(!(empty($admin['roleid']) || (($admin['roleid'] instanceof \think\Collection || $admin['roleid'] instanceof \think\Paginator ) && $admin['roleid']->isEmpty()))): if(is_array($role) || $role instanceof \think\Collection || $role instanceof \think\Paginator): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo htmlentities($row['id']); ?>" <?php if($admin['roleid'] == $row['id']): ?>selected="selected"<?php endif; ?>><?php echo htmlentities($row['rolename']); ?></option>
                        <?php endforeach; endif; else: echo "" ;endif; else: if(is_array($role) || $role instanceof \think\Collection || $role instanceof \think\Paginator): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo htmlentities($row['id']); ?>"><?php echo htmlentities($row['rolename']); ?></option>
...
<script>
    ...
    function addadmin(){
        ...
        ajax('<?php echo url("Account/doadmin"); ?>', {adminname:adminname,password:password,roleid: roleid,id:id}, function(res){
